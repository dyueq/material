...
export default {
    ...
    methods: {
        getAll() {
            //调后端分页查询该用户所有捐赠记录的方法
            this.$axios.get(`/getUserRecord/${this.page.currentPage}/${this.page.size}`)
                .then((res) => {
        ...
        onSubmit() {
            ...
            this.$axios.post(`/getUserRecordsBy/${this.page.currentPage}/${this.page.size}`, this.recordForm)
                .then((res) => {
                    console.log(res);
                    this.tableData = res.data.data.records;
                    this.page.size = res.data.data.size;
